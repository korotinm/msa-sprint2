...
echo "🏁 Регрессионный тест Hotelio (после выноса booking в микросервис)"

# Хосты БД: при запуске внутри сети hotelio-net задаются именами сервисов
# (booking-db / booking-history-db), при запуске с хоста — падают на общий DB_HOST.
BOOKING_DB_HOST="${BOOKING_DB_HOST:-$DB_HOST}"
HISTORY_DB_HOST="${HISTORY_DB_HOST:-$DB_HOST}"

# Проверка соединения
echo "🧪 Проверка подключения к БД..."
for pair in "${DB_HOST}:${DB_PORT}" "${BOOKING_DB_HOST}:${BOOKING_DB_PORT}" "${HISTORY_DB_HOST}:${HISTORY_DB_PORT}"; do
  host="${pair%:*}"; port="${pair##*:}"
  timeout 2 bash -c "</dev/tcp/${host}/${port}" \
    || { echo "❌ Не удалось подключиться к ${host}:${port}"; exit 1; }
done
...
echo "booking-service — очистка bookings"
PGPASSWORD="$BOOKING_DB_PASSWORD" psql -h "$BOOKING_DB_HOST" -p "$BOOKING_DB_PORT" \
  -U "$BOOKING_DB_USER" "$BOOKING_DB_NAME" -c "TRUNCATE bookings RESTART IDENTITY;"

echo "booking-history — очистка booking_history"
PGPASSWORD="$HISTORY_DB_PASSWORD" psql -h "$HISTORY_DB_HOST" -p "$HISTORY_DB_PORT" \
  -U "$HISTORY_DB_USER" "$HISTORY_DB_NAME" -c "TRUNCATE booking_history;"
...
echo ""
echo "Тесты бронирования (монолит -> gRPC booking-service)..."
# Операции "получить все брони" в контракте ListBookings(user_id) нет.

# 1. Успешное бронирование без промо
curl -sSf -X POST "${BASE}/api/bookings?userId=test-user-3&hotelId=test-hotel-1" | grep -q 'test-hotel-1' \
  && pass "Бронирование прошло (без промо)" || fail "Бронирование (без промо) не прошло"

# 2. Успешное бронирование с промо
curl -sSf -X POST "${BASE}/api/bookings?userId=test-user-2&hotelId=test-hotel-1&promoCode=TESTCODE1" | grep -q 'TESTCODE1' \
  && pass "Бронирование с промо прошло" || fail "Бронирование с промо не прошло"

# 3. Листинг по пользователю (REST монолита -> proxy -> gRPC booking-service)
curl -sSf "${BASE}/api/bookings?userId=test-user-2" | grep -q 'test-user-2' \
  && pass "Листинг броней test-user-2" || fail "Нет броней test-user-2"
...
[[ "$code" == "500" ]] && pass "Отклонено: неактивный пользователь" \
  || fail "Сервер принял бронирование от неактивного пользователя (код $code)"
...
[[ "$code" == "500" ]] && pass "Отклонено: недоверенный отель" \
  || fail "Сервер принял бронирование от недоверенного отеля (код $code)"
...
[[ "$code" == "500" ]] && pass "Отклонено: отель полностью забронирован" \
  || fail "Сервер принял бронирование в полностью занятом отеле (код $code)"
